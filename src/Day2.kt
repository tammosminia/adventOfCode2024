...
object Day2 {
    ...
    fun parse(input: String): Input =
        input.lines().filter { it.isNotBlank() }.map { parseLine(it) }

    fun run1(input: Input): Int =
        input.count { isSafe(it) }

    fun isSafe(r: Report): Boolean {
        fun isIncreasing(): Boolean = r.windowed(2).all { it[0] < it[1] }
        fun isDecreasing(): Boolean = r.windowed(2).all { it[0] > it[1] }
        fun safeAmounts(): Boolean = r.windowed(2).all { abs(it[0] - it[1]) < 4 }

        return safeAmounts() && (isIncreasing() || isDecreasing())
    }

    fun run2(input: List<List<Int>>): Int =
        input.count { problemDampener(it) }

    fun<E> List<E>.removeIndex(index: Int): List<E> =
        this.subList(0, index) + this.subList(index + 1, this.size)

    fun problemDampener(r: Report): Boolean {
        val permutations = List(r.size) { index -> r.removeIndex(index) }.plusElement(r)
