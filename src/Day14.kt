...
object Day14 {
    ...
    data class RobotRoom(val robots: List<Robot>, val width: Int, val height: Int) {
        ...
        fun quadrantOf(c: Coordinate): Int? {
            ...
            val right = c.x > width / 2
            return if (top && left) 1
                else if (top && right) 2
                else if (bottom && left) 3
                else if (bottom && right) 4
                else null
        }


        fun safetyFactor(): Int =
            robots.groupBy { quadrantOf(it.pos) }
                .minus(null)
    ...
    fun parse(input: String, width: Int, height: Int): RobotRoom =
        RobotRoom(input.lines().map(Robot::parse), width, height)

    tailrec fun simulate(rr: RobotRoom, moves: Int = 100): RobotRoom =
        if (moves == 0) rr
...
fun main() {
    val input = """
        ...
        p=43,101 v=56,3
    """.trimIndent()
    val parsed = parse(input, 101, 103)
    println(run1(parsed))
    println(run2(parsed))
}
